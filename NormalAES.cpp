...
//#pragma omp parallel
   ...
   // Get the input text from file
    fstream inpfile;
    FILE* en_fp;
      en_fp = fopen("encrypt.txt", "wb");

    inpfile.open("novel.txt", ios::in);
    //encryptedfile.open("encryptedfile.txt", ios::out);
   ...
   while (sz < strlen(str)) {
      ...
      for (i=0 ; i < Nb*4 ; i++)  { 
                                                          cc++;
                                                         if(cc == 16){ fprintf(en_fp, "\n"); cc=0;
                                                                     }
                                                         fprintf(en_fp, "%02x ", out[i]);
                }
   }
   //printf("\n\n");
}
